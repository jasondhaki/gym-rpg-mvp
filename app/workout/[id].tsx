...
export default function ActionChamberScreen() {
  ...
  // 1. SESSION SYNC: Isolated tracking per questId
  useFocusEffect(
    useCallback(() => {
      const syncSession = async () => {
        ...
        const thisQuestCompleted = (currentQuestId && questMap[currentQuestId]) 
          ? questMap[currentQuestId] 
          : [];

        setCompletedCount(thisQuestCompleted.length);

        // FILTER: Keep non-synced exercises visible in the available operations pool
        setActiveExercises(prev => prev.filter(ex => !thisQuestCompleted.includes(ex.id)));
      };
  ...
  // 2. INITIAL RANDOMIZER: Fills the board with options
  useEffect(() => {
    if (quest && activeExercises.length === 0) {
      ...
      // Providing a 10-exercise buffer so the user can go beyond the goal of 5
  ...
  // Visual Cap: The bar stops at 100% (5/5) even if completedCount is higher
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.dataContainer}>
          ...
          {/* SUCCESS BADGE: Only appears after hitting the goal */}
          {completedCount >= GOAL && (
          ...
          {/* EXERCISE LIST: Stays visible as long as there are items in activeExercises */}
...
const styles = StyleSheet.create({
  ...
  progressLabel: { color: '#71717a', fontSize: 10, letterSpacing: 2, fontFamily: 'SpaceMono' },
  progressValue: { color: 'white', fontSize: 14, fontWeight: 'bold', fontFamily: 'SpaceMono' },
  ...
  successText: { fontSize: 10, fontWeight: 'bold', fontFamily: 'SpaceMono', letterSpacing: 1 },
  ...
  emptyStateText: { color: '#71717a', marginTop: 15, fontFamily: 'SpaceMono', letterSpacing: 2, fontSize: 10 }
});